...
| PCがオンで操作可能 | **PCの再起動・シャットダウンができます。** | `/status` |
| PCがオフで操作可能 | **PCの起動ができます。** | `/status` |
| PCがオンで操作サービス応答なし | **再起動・シャットダウンはできません。** | 次行に `(PC側の操作サービスが応答していません)` |
| 操作ロック中 | **操作をロックしています。** | 次行に `解除するまでPCの起動・再起動・シャットダウンはできません。`。PC・操作サービスの状態より優先 |
| 端末情報の見出し | **M5Stack** | PCブロックとの間に空行を1つ置き、バッテリー行とバージョン行をまとめる |
| バッテリーの状態 | **残量%の直後に括弧で給電状態** | `バッテリー: 100%(満充電・給電中)` / `78%(電池駆動)` / `95%(充電中)`。未取得は `バッテリー: 不明` |
| 端末のバージョン | **バージョン: {version}** | `バージョン: 0.12.0` |
...
- 文言を組み立てる関数はできるだけ1箇所に寄せる。
  ...
  `wake_succeeded_text` / `wake_timed_out_text` が正本。電源操作の結果文は
   ...
   `/status` の全文は同crateの `status_text_ja` が正本。
