...
- `TELEGRAM_BOT_TOKEN` と `TELEGRAM_ALLOWED_USER_ID` は `firmware/config.toml` に置き、Windows Agent用の `AGENT_SHARED_SECRET` とは分離する。Telegramや外部中継先には `AGENT_SHARED_SECRET` を渡さない。
...
- 確認実行時はWindows Agent向けHMAC署名付きPOSTを使うため、Windows Agent側の `confirm: true` 必須条件・timestamp・nonce検証はTelegram経由でも同様に効く。
