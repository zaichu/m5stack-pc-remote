...
//! ソース側で無条件importしているため)。待機状態の管理と期限判定だけをここへ分離
//! することで、実機なしでhost側のテストを回せるようにする
//! (`battery` の `PowerNotify` と同じ方針。Issue #182)。
//!
//! 時刻(`Instant`)は扱わない。`firmware` 側が `Instant`(単調時計)で測った
//! 経過秒だけを受け取る。NTP(壁時計)に依存しないため、未同期時でも壊れない。
//! 共有状態(`SharedWakeWatch`)の型とロックは `firmware/src/telegram.rs` が持ち、
//! ここにはI/Oと呼び出しだけが残る。

/// 起動指示後にPCがオンになるのを待つ期限(秒)。
///
/// 3分とした根拠: 一般的なWindowsの起動(電源投入からOS起動・bridge上昇まで)は
/// 1〜2分程度で、STATUS確認周期10秒×安定2回(20秒)とbridge起動のばらつきを足すと
/// 2分半ばになる。短すぎると「遅いだけ」で誤警告になり、長すぎると気づくのが
/// 遅れるため、余裕を見て3分にする。**実機では測っていない**ため、実測で
/// ずれる場合はこの定数だけを変える(判定式は `>=` のまま変えない)。
